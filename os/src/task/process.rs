...
pub struct ProcessControlBlockInner {
    ...
    pub killed: bool,
    /// if the process is frozen by a signal
    pub frozen: bool,
...
impl ProcessControlBlock {
    ...
    /// Only support processes with a single thread for exec
    pub fn exec(self: &Arc<Self>, elf_data: &[u8], args: Vec<String>) {
        ...
        *task_inner.get_trap_cx() = trap_cx;
        // update heap_bottom and program_brk
        let mut process_inner = self.inner_exclusive_access();
        process_inner.heap_bottom = ustack_base;
        process_inner.program_brk = ustack_base;
    }

    /// Only support processes with a single thread for fork
    pub fn fork(self: &Arc<Self>) -> Arc<Self> {
        ...
        let task = Arc::new(TaskControlBlock::new(
            ...
            // but we allocate a new kstack here
            false,
        ));
